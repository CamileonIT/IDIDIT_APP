...
import '../data/models/addiction.dart';
...
class DBHelper {
  ...
  Future<Addiction> add(Addiction student) async {
  ...
  Future<List<Addiction>> getStudents() async {
    ...
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        students.add(Addiction.fromMap(maps[i]));
      }
    }
    return students;
  }
  ...
  Future<int> update(Addiction student) async {
    var dbClient = await db;
